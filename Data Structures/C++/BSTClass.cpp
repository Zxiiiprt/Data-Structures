...
class BST
{
  public:
    ...
    /* Returns a Pointer to a Node */ // NON RECURSIVE WAY JUST PICK WHICHEVER YOU WANT - If you can fix the
                                      // Segmentation Faults caused by the recursive method then go ahead
    ...
    /* Returns a Pointer to a Node */ // Found a recursive way to search afterwards BUT for some reason was
                                      // giving Segmentation faults (will debug another time)
    Node *Search(Node *rt, int value)
    {
        if (rt->value == value || rt == NULL)
        {
            return rt;
        }
        else if (value > rt->value)
...
void printMenu(BST bst)
{
    ...
    while (choice != 5)
    {
        std::cout << "Choose how you want to Print your BST (1-5):" << endl
    ...
    while (option != 5)
    {
        ...
        switch (option)
        {
            ...
            case 2:
                ...
                if (node != NULL)
                {
                    std::cout << "The value " << value << " exists in the Tree" << endl << "\n";
                }
                else
                {
                    std::cout << value << " DOES NOT exist in the Tree currently" << endl << "\n";
                }
                std::this_thread::sleep_for(std::chrono::milliseconds(200));
                break;
            case 3:
                ...
                {
                    std::cout << "The value " << value << " DOES NOT exist in the Tree" << endl << "\n";
                }
                std::this_thread::sleep_for(std::chrono::milliseconds(200));
                break;
            case 4:
